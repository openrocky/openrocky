...
class MicrophonePCMSampleVendorAE: MicrophonePCMSampleVendor {
  init(audioEngine: AVAudioEngine) throws {
    ...
    // Enable voice processing (AEC) when not using headphones.
    // NOTE: This alters the AVAudioEngine's internal audio graph.
    // The AudioPCMPlayer must connect to mainMixerNode (not outputNode)
    // to avoid sample rate conversion issues caused by voice processing.
